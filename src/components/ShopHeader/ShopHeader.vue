<template>
  <div class="shop-header">
    ...
    <div class="shop-header-discounts" @click="showSupport()">
      <div class="discounts-left">
        <div class="activity activity-green">
          <span class="content-tag">
          <span class="mini-tag" v-if="shopInfo.supports">{{shopInfo.supports[0].name}}</span>
          </span>
          <span class="activity-content ellipsis" v-if="shopInfo.supports">{{shopInfo.supports[0].content}}</span>
        </div>
      </div>
      <div class="discounts-right" v-if="shopInfo.supports">
    ...
    <div class="activity-sheet" v-show="isShow">
      <div class="activity-sheet-content">
        <h2 class="activity-sheet-title">优惠活动</h2>
        <div class="supportsContent">
          <ul class="list">
            <li class="activity-item activity-green" :class="classArray[support.type]" v-for="(support, index) in shopInfo.supports" :key="index">
            <span class="content-tag">
      ...
      <div class="activity-sheet-cover" @click="isShow=false"></div>
    </div>
  </div>
</template>
...
export default {
  data () {
    return {
      classArray: ['activity-green', 'activity-red', 'activity-orange'],
      isShow: false
    }
  },
  mounted () {
    this.recevieShopInfo()
  },
  computed: {
    ...mapState(['shopInfo'])
  },
  methods: {
    ...mapActions(['recevieShopInfo']),
    showSupport () {
      this.isShow = true
      if (!this.scroll) {
        this.$nextTick(() => {
          this.scroll = new BScroll('.supportsContent', {
            click: true
          })
        })
      }
